...
public class HelloADKActivity extends Activity implements Runnable, OnSeekBarChangeListener {
            ...
            public void run() {
                switch (servoIndex) {
                    case 0:
                        ...
                        if (value != -1) {
                            HelloADKActivity.this.incoming1SeekBar.setProgress(value);
                        }
                        break;
                    case 2:
                        HelloADKActivity.this.incoming2EditText.setText("" + value);
                        if (value != -1) {
                            HelloADKActivity.this.incoming2SeekBar.setProgress(value);
                        }
                        break;
    ...
    public void run() {
        ...
        while (ret >= 0) {
            ...
            Log.d(TAG, "buffer[0] is:" + buffer[0] + " buffer[1] is:" + buffer[1] + " buffer[2] is:" + buffer[2]);
            switch (buffer[0]) {
                case 1:
                case 2:
                    incoming = buffer[1];
                    incoming += buffer[2] * 256;
                    Log.d(TAG, "current pwm is " + incoming);
                    HelloADKActivity.this.updateIncomingSeekBar(incoming, buffer[0] - 1);
                    break;
                default:
    ...
    public void onProgressChanged(SeekBar arg0, int progress, boolean arg2) {
        byte highProgress = (byte) (progress >> 0xff);
        byte lowProgress = (byte) (progress & 0xff00);
        if (arg0 == incoming1SeekBar) {
            sendCommand(SERVO1_COMMAND, lowProgress, highProgress);
        } else if (arg0 == incoming2SeekBar) {
